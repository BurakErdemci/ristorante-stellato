...
interface GalleryItem {
  src: string;
  alt: string;
  titleKey: keyof typeof import('@/i18n/tr').default.gallery.items;
  className: string;
}
        ...
        <div className="flex flex-col md:flex-row justify-between items-end mb-16 gap-6">
          <div>
            ...
            <motion.h2
              ...
              viewport={{ once: true }}
              className="font-serif text-4xl md:text-6xl text-stellato-cream leading-tight"
            >
              Stellato <span className="italic text-theme-muted">{t.gallery.title.replace('Stellato ', '').replace("L'Esperienza Stellato", 'Experience').replace('Stellato Deneyimi', 'Deneyimi')}</span>
            </motion.h2>
          </div>
          ...
          <motion.a
            ...
            viewport={{ once: true }}
            className="flex items-center gap-2 text-theme-muted hover:text-stellato-gold transition-colors group pb-2"
          >
            <Instagram size={20} />
            <span className="text-xs uppercase tracking-widest">{t.gallery.followCta}</span>

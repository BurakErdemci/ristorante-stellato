...
export default function About() {
  ...
  return (
    <>
      {/* --- ABOUT SECTION --- */}
      <section id="hikayemiz" className="py-24 md:py-32 bg-elevated relative overflow-hidden">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-20 items-center">
            ...
            >
              ...
              <div className="relative z-10 overflow-hidden shadow-2xl">
                ...
                {/* Chef Badge */}
                <div className="absolute bottom-6 right-6 bg-stellato-black/80 backdrop-blur-md p-4 md:p-6 border border-theme-border shadow-xl text-right">
                  <div className="flex justify-end gap-1 mb-2">
                    {[1,2,3].map(i => (
                      <Star key={i} size={12} fill="#D4AF37" className="text-stellato-gold" />
                    ))}
                  </div>
                  <p className="font-serif text-lg md:text-xl text-stellato-cream leading-none mb-1">Giovanni Rossi</p>
            ...
            >
              ...
              <h2 className="font-serif text-4xl md:text-6xl text-stellato-cream mb-8 leading-tight">
                {t.about.title1} <br />
                <span className="italic text-stellato-gold opacity-90">{t.about.title2}</span>
              </h2>

              <div className="space-y-6 text-theme-muted font-light leading-8 text-base md:text-lg text-justify">
                <p>{t.about.paragraph1}</p>
                <p>{t.about.paragraph2}</p>
              </div>

              {/* İMZA & ÖDÜL ALANI */}
              <div className="mt-12 flex flex-col md:flex-row items-start md:items-center gap-8 md:gap-8 border-t border-theme-border pt-8">

                {/* İmza */}
                <div className="flex flex-col gap-2">
                   <span className="font-serif italic text-3xl text-theme-muted select-none">Giovanni Rossi</span>
                </div>
                ...
                {/* Michelin Badge */}
                <div className="flex items-center gap-4 group cursor-default w-full md:w-auto bg-theme-overlay md:bg-transparent p-3 md:p-0 rounded md:rounded-none border border-theme-border-faint md:border-none">
                  ...
                  <div>
                    <p className="font-serif text-base text-stellato-cream">{t.about.badge1}</p>
                    <p className="text-[10px] text-theme-muted uppercase tracking-widest group-hover:text-stellato-gold transition-colors">{t.about.badge2}</p>
                  </div>
                </div>
      ...
      >
        ...
        >
            ...
            <cite className="block mt-8 text-xs md:text-sm text-theme-secondary uppercase tracking-[0.2em] not-italic">
                — {t.about.quoteAuthor}
            </cite>
        </motion.div>
      </div>
